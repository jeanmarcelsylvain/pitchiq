...
-- ═══ Tracker Integrations (Veo, Wyscout, Hudl, etc.) ═══════════════════════
-- OAuth connections and sync state for each tracker per user
CREATE TABLE IF NOT EXISTS tracker_integrations (
  id              UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  user_id         TEXT NOT NULL,
  tracker_type    TEXT NOT NULL CHECK (tracker_type IN ('veo','wyscout','hudl','kinexon','playermaker')),
...
CREATE INDEX IF NOT EXISTS tracker_integrations_user_id_idx ON tracker_integrations(user_id);
CREATE INDEX IF NOT EXISTS tracker_integrations_tracker_type_idx ON tracker_integrations(tracker_type);

-- Sync history for debugging and monitoring
CREATE TABLE IF NOT EXISTS tracker_sync_history (
  id              UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  user_id         TEXT NOT NULL,
  tracker_type    TEXT NOT NULL,
  sync_type       TEXT NOT NULL CHECK (sync_type IN ('full','incremental','partial')),
  status          TEXT NOT NULL CHECK (status IN ('pending','success','failed')),
  matches_synced  INT NOT NULL DEFAULT 0,
...
CREATE INDEX IF NOT EXISTS tracker_sync_history_user_id_idx ON tracker_sync_history(user_id);
CREATE INDEX IF NOT EXISTS tracker_sync_history_tracker_type_idx ON tracker_sync_history(tracker_type);
CREATE INDEX IF NOT EXISTS tracker_sync_history_status_idx ON tracker_sync_history(status);

-- Extended matches table columns to link to tracker sources
ALTER TABLE matches ADD COLUMN IF NOT EXISTS
  tracker_source TEXT CHECK (tracker_source IN ('veo','wyscout','hudl','kinexon','playermaker','manual'));

ALTER TABLE matches ADD COLUMN IF NOT EXISTS
  tracker_match_id TEXT;

ALTER TABLE matches ADD COLUMN IF NOT EXISTS
  tracker_sync_id UUID REFERENCES tracker_sync_history(id);

CREATE INDEX IF NOT EXISTS matches_tracker_source_idx ON matches(tracker_source);
CREATE INDEX IF NOT EXISTS matches_tracker_match_id_idx ON matches(tracker_match_id);
