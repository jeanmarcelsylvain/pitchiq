...
import { Check, ChevronLeft, ChevronRight, Sparkles, X, Zap, ListChecks } from 'lucide-react'
  ...
  return (
    <motion.div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-0 sm:p-4"
      ...
      <motion.div
        ...
        className="w-full sm:max-w-2xl max-h-[92vh] overflow-y-auto rounded-t-2xl sm:rounded-2xl shadow-2xl"
...
function FlowHeader({ title, subtitle, onClose, children }: { title: string; subtitle: string; onClose: () => void; children?: React.ReactNode }) {
  return (
    <div className="flex items-start gap-3 border-b px-6 py-4" style={{ borderColor: color.border }}>
      <div className="min-w-0">
        ...
        <p style={{ ...B, fontSize: '0.75rem', color: color.inkMuted }} className="mt-0.5">{subtitle}</p>
      </div>
      {children}
      <button onClick={onClose} aria-label="Close" style={{ color: color.inkMuted }} className="hover:text-white transition-colors shrink-0">
        <X className="h-5 w-5" />
      </button>
    </div>
  )
...
function SegmentedControl<T extends string>({ value, onChange, options }: { value: T; onChange: (v: T) => void; options: [T, string][] }) {
  return (
...
function NumberInput({ value, onChange, max, step = 1 }: { value: number; onChange: (v: number) => void; max?: number; step?: number }) {
  return (
